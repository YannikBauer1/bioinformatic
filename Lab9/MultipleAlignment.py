...
class MultipleAlignment():
    ...
    def ScoreColumn(self, charsCol):
        ...
        for i in range(len(charsCol)):
            for j in range(i+1,len(charsCol)):
                if charsCol[i]=="-" and charsCol[j]=="-":
                    continue
                else:
                    #print(self.alignpars.score_pos(charsCol[i],charsCol[j]))
                    #print(charsCol[i],charsCol[j])
                    score=score + self.alignpars.score_pos(charsCol[i],charsCol[j])
        return score
    ...
    def scoreSP (self, alignment):
        ...
        for i in range(len(alignment)):
            ...
            score = score + self.ScoreColumn(col)
        return score
